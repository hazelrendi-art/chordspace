...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    const song = { id, title: String(title).trim(), artist: String(artist || '').trim(), difficulty: difficulty || 'Beginner', description: String(description || '').trim(), content: String(content).trim() };
    songs.push(song);
